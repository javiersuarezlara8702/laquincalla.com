...
class LaquincallaInsumosSupabaseUser extends BaseAuthUser {
  LaquincallaInsumosSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> laquincallaInsumosSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LaquincallaInsumosSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
